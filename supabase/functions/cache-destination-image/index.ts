...
const isSupabaseDestinationImageUrl = (url: string) => {
...
Deno.serve(async (req) => {
  ...
  const name = safeString(payload.name || "destination");
  const location = safeString(payload.location || "");
  const key = safeString(payload.key || `${name}-${location}`);
  if (!imageUrl) return json({ ok: false, error: "imageUrl is required." }, 400);
  if (imageUrl.startsWith("data:")) return json({ ok: true, imageUrl });
  if (isSupabaseDestinationImageUrl(imageUrl)) return json({ ok: true, imageUrl });
  ...
  try {
    const sourceResponse = await fetch(parsedUrl.toString(), {
      headers: {
        ...
        "User-Agent": "our-calendar-destination-image-cache/1.0",
      },
    });
    ...
    const objectPath = `destinations/${slugify(key || `${name}-${location}`)}-${hash}.${ext}`;
    ...
    console.error("cache-destination-image failed", error);
