...
const ActionPill = ({ href, onClick, children }) => {
  const className = 'inline-flex items-center gap-1.5 rounded-full border border-rose-200 bg-white/92 px-3.5 py-1.5 text-xs font-semibold text-rose-700 shadow-sm transition-all hover:border-rose-300 hover:bg-rose-50/75 hover:shadow-md active:scale-[0.98] dark:border-rose-400/20 dark:bg-white/5 dark:text-rose-200 dark:hover:bg-white/10';
      ...
      <div className="relative px-6 py-7 sm:px-7">
        {(props.onEdit || props.onDelete) ? (
          <div className="absolute right-6 top-5 z-10 flex items-center gap-2 sm:right-7">
            {props.onEdit ? (
              <button className="rounded-full border-2 border-rose-200 bg-white/95 p-2.5 text-rose-600 shadow-sm transition-all hover:border-rose-300 hover:text-rose-800 hover:shadow-md dark:border-rose-400/20 dark:bg-white/5 dark:text-rose-200 dark:hover:bg-white/10 dark:hover:text-white" onClick={props.onEdit} type="button">
                <EditIcon />
              </button>
            ) : null}
            {props.onDelete ? (
              <button className="rounded-full border-2 border-rose-200 bg-white/95 p-2.5 text-rose-600 shadow-sm transition-all hover:border-red-200 hover:bg-red-50 hover:text-red-600 hover:shadow-md dark:border-rose-400/20 dark:bg-white/5 dark:text-rose-200 dark:hover:border-red-400/20 dark:hover:bg-red-500/10 dark:hover:text-red-400" onClick={props.onDelete} type="button">
                <TrashIcon />
              </button>
            ) : null}
          </div>
        ...
        <div className={`relative mx-auto max-w-[30rem] rounded-[28px] border bg-white/78 px-6 py-7 text-center shadow-[0_18px_40px_rgba(15,23,42,0.08)] backdrop-blur-[10px] dark:backdrop-blur-[12px] ${tone.sectionBorder || 'border-rose-200/80 dark:border-rose-400/15'} ${tone.kind === 'baby' ? 'dark:bg-[rgba(23,34,48,0.72)]' : 'dark:bg-[rgba(38,28,57,0.72)]'}`}>
          ...
          <div className="mb-3 flex items-center justify-center">
            <div className={`text-[11px] font-bold uppercase tracking-[0.32em] ${tone.detailLabel || 'text-rose-700 dark:text-rose-200'}`}>
              You're Invited
            </div>
          </div>
          ...
          <div className={`mx-auto mt-4 h-px w-24 bg-gradient-to-r from-transparent ${tone.kind === 'baby' ? 'via-sky-400 dark:via-sky-300/80' : 'via-rose-400 dark:via-rose-300/80'} to-transparent`} />
