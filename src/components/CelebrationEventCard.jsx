...
const Section = ({ title, subtitle, actions, children }) => (
  <div className="group/section rounded-[22px] border border-rose-200/70 bg-white/90 p-5 shadow-[0_12px_30px_rgba(244,63,94,0.09)] backdrop-blur-sm transition-all hover:shadow-[0_18px_40px_rgba(244,63,94,0.14)] dark:border-rose-400/14 dark:bg-white/[0.06] dark:shadow-none dark:hover:bg-white/[0.08]">
...
const EmptySection = ({ title, subtitle, actions }) => (
  <Section title={title} subtitle={subtitle} actions={actions}>
    <div className="rounded-2xl border border-dashed border-rose-200 bg-rose-50/70 px-4 py-4 text-sm text-rose-700 dark:border-rose-400/16 dark:bg-rose-500/8 dark:text-rose-200">
      Nothing added yet.
    </div>
  </Section>
);
...
const InviteeRow = ({ event, label = 'Attending' }) => {
        ...
        {invitees.length === 0 ? (
          ...
          invitees.slice(0, 8).map((invitee, index) => (
            <div
              ...
              className="inline-flex items-center gap-2 rounded-full border border-rose-200 bg-white px-3.5 py-2 text-xs font-semibold text-gray-700 shadow-sm dark:border-rose-400/14 dark:bg-white/5 dark:text-gray-200"
            >
...
const CelebrationEventCard = ({ event, onUpdateEventData, onEdit, openEditor, ...props }) => {
  ...
  return (
    <div className="group relative w-full overflow-hidden rounded-[32px] border-2 border-rose-300/70 bg-gradient-to-br from-rose-50 via-pink-50/80 to-orange-50/50 shadow-[0_24px_80px_rgba(244,63,94,0.25)] transition-all duration-300 hover:shadow-[0_28px_100px_rgba(244,63,94,0.35)] dark:border-rose-400/25 dark:from-[#2d1a1f] dark:via-[#1e1517] dark:to-[#14110f] dark:shadow-none">
      ...
      <div className="pointer-events-none absolute -right-12 -top-12 h-32 w-32 rotate-45 bg-gradient-to-br from-rose-400/20 to-pink-400/10 dark:from-rose-500/10 dark:to-pink-500/5" style={{ animation: 'celebration-shimmer 3s ease-in-out infinite' }} />

      <div className="pointer-events-none absolute inset-0">
        <div className="absolute left-[20%] top-[25%] h-1 w-1 rotate-45 bg-rose-400 opacity-60 dark:opacity-30" style={{ animation: 'celebration-twinkle 4s ease-in-out infinite' }} />
        <div className="absolute right-[25%] top-[60%] h-1.5 w-1.5 rotate-12 bg-pink-400 opacity-50 dark:opacity-25" style={{ animation: 'celebration-twinkle 3.5s ease-in-out infinite 1s' }} />
        <div className="absolute left-[70%] top-[40%] h-1 w-1 rotate-[30deg] bg-orange-400 opacity-55 dark:opacity-28" style={{ animation: 'celebration-twinkle 4.5s ease-in-out infinite 2s' }} />
      </div>

      <div className="relative border-b-2 border-rose-200/80 bg-gradient-to-br from-white/95 to-rose-50/60 px-6 py-6 dark:border-rose-400/15 dark:from-white/[0.04] dark:to-rose-500/[0.02] sm:px-7">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <div className="mb-4 flex flex-wrap items-center gap-2.5">
              <span
                className="inline-flex h-12 w-12 items-center justify-center rounded-[18px] border-2 border-rose-300 bg-gradient-to-br from-rose-100 to-pink-100 text-2xl shadow-lg shadow-rose-200/40 transition-transform group-hover:scale-110 dark:border-rose-400/30 dark:from-rose-500/15 dark:to-pink-500/15 dark:shadow-rose-500/10"
                style={{ animation: 'celebration-gentle-float 4s ease-in-out infinite' }}
              >
                Celebrate
              </span>

              <span className="rounded-full bg-gradient-to-br from-rose-100 to-rose-200 px-3 py-1.5 text-[10.5px] font-bold uppercase tracking-[0.18em] text-rose-800 shadow-sm dark:from-rose-500/15 dark:to-rose-600/15 dark:text-rose-200">
                Celebration
              </span>

              {dressCode ? (
                <span className="rounded-full border-2 border-rose-200 bg-white/90 px-3 py-1.5 text-xs font-semibold text-rose-700 shadow-sm dark:border-rose-400/20 dark:bg-white/5 dark:text-rose-200">
                  {dressCode}
                </span>
              ) : null}
            </div>
      ...
      <div className="relative space-y-5 px-6 py-6 sm:px-7">
        ...
        >
          <div className="relative overflow-hidden rounded-2xl border-2 border-rose-100 bg-white p-4 shadow-sm dark:border-rose-500/10 dark:bg-white/5">
            <div className="flex items-start gap-3">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-rose-100 to-pink-100 text-sm font-semibold text-rose-700 dark:from-rose-500/15 dark:to-pink-500/15 dark:text-rose-200">
                Style
              </div>
              <div className="min-w-0 flex-1">
                <div className="mb-1 text-xs font-bold uppercase tracking-[0.14em] text-rose-600 dark:text-rose-300">
                  Dress Code
                </div>
        ...
        {schedule.length > 0 ? (
          ...
          >
            <div className="relative space-y-3">
              <div className="absolute left-[19px] top-4 bottom-4 w-[2px] bg-gradient-to-b from-rose-200 via-pink-200 to-rose-200 dark:from-rose-500/20 dark:via-pink-500/20 dark:to-rose-500/20" />
              {schedule.map((item, index) => (
                <div key={`schedule-${index}`} className="relative flex items-start gap-3 pl-1">
                  ...
                  <div className="min-w-0 flex-1 rounded-xl border-2 border-rose-100 bg-white p-3 shadow-sm dark:border-rose-500/10 dark:bg-white/5">
                    <div className="mb-1 text-xs font-bold text-rose-600 dark:text-rose-300">
                      {item?.time || 'Time TBD'}
                    </div>
                    <div className="text-sm font-semibold text-gray-900 dark:text-white">
