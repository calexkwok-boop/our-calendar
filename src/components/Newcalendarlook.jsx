...
const GreetingHeader = ({ todayEvents, activeTrips, openSubCalendar, onScrollToTodaySchedule, darkMode, user, userName, accent }) => {
  ...
  return (
    <div
      className="mb-6 rounded-[28px] p-5 shadow-lg border"
      style={{
        background: darkMode
          ? 'linear-gradient(135deg, rgba(120,100,60,0.18) 0%, rgba(15,23,42,0.85) 100%)'
          : 'linear-gradient(135deg, #fdf8f0 0%, #fef9f2 100%)',
        borderColor: darkMode ? 'rgba(255,255,255,0.10)' : 'rgba(180,150,100,0.25)',
      }}
    >
      <div className="flex items-center justify-between">
        {/* Left: Greeting */}
        <div className="flex items-center gap-3">
          <span className="text-4xl sm:text-5xl">{getTimeBasedEmoji()}</span>
          <div>
            <button
              type="button"
        ...
        <div className="flex flex-col items-end gap-2">
          {todayEvents.length > 0 && (
            <button
              type="button"
              onClick={() => onScrollToTodaySchedule?.()}
              className="flex items-center gap-2 rounded-full px-3 py-1.5 shadow-sm transition-all hover:shadow-md"
              style={{ background: darkMode ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.85)', border: '0.5px solid rgba(180,150,100,0.3)' }}
            >
              <span className="text-base">📅</span>
              <span className="text-sm font-semibold" style={{ color: darkMode ? '#e5d9c8' : '#5a4a3a' }}>
          ...
          {activeTrips.length === 1 && (
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-full shadow-sm hover:shadow-md hover:scale-105 transition-all"
              style={{ background: 'linear-gradient(135deg, #93c5fd 0%, #6ee7b7 100%)', color: '#1e3a5f' }}
            >
              <span className="text-sm">✈️</span>
              <span className="text-xs font-semibold truncate max-w-[120px]">{activeTrips[0].name}</span>
            </button>
          )}
          {activeTrips.length > 1 && (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full shadow-sm" style={{ background: 'linear-gradient(135deg, #93c5fd 0%, #6ee7b7 100%)', color: '#1e3a5f' }}>
              <span className="text-sm">✈️</span>
  ...
  <div className="mb-6 space-y-3">
    {trips.map(trip => (
      <button
        ...
        className="group w-full relative overflow-hidden rounded-[28px] shadow-lg hover:shadow-2xl transition-all duration-300"
        style={{ border: '1px solid rgba(180,150,100,0.3)' }}
      >
        <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, #bfdbfe 0%, #a7f3d0 100%)' }} />
        ...
        <div className="relative z-10 flex items-center gap-4 p-5 text-left">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center shadow-md group-hover:scale-110 transition-transform" style={{ background: 'rgba(255,255,255,0.85)' }}>
            <span className="text-3xl">✈️</span>
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <div className="px-2 py-0.5 rounded-full text-[10px] font-bold" style={{ background: 'rgba(255,255,255,0.85)', color: '#0e7490' }}>
                HAPPENING NOW
              </div>
            </div>
          ...
          <div className="text-2xl group-hover:translate-x-2 transition-transform" style={{ color: '#1e3a5f' }}>→</div>
        </div>
      </button>
    ))}
  </div>
    ...
    <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex items-center justify-center gap-2 sm:justify-start">
        <button
          ...
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl shadow-sm transition hover:-translate-y-0.5 hover:shadow-md sm:h-10 sm:w-10"
          style={{ background: darkMode ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.85)', border: '0.5px solid rgba(180,150,100,0.25)', color: darkMode ? '#e5d9c8' : '#5a4a3a' }}
        >
      ...
      {/* View toggle */}
      <div
        className="inline-flex w-full items-center justify-center gap-1 self-center rounded-2xl p-1 sm:w-auto sm:justify-end sm:self-auto"
        style={{ background: darkMode ? 'rgba(255,255,255,0.06)' : 'rgba(245,235,220,0.7)', border: '0.5px solid rgba(180,150,100,0.2)' }}
      >
        ...
        return (
          ...
          style={{
            background: isSelected
              ? 'linear-gradient(135deg, #c4a882 0%, #a08060 100%)'
              : isTodayDate
                ? (darkMode ? 'rgba(180,150,100,0.22)' : 'rgba(245,235,215,0.95)')
                : (darkMode ? 'rgba(31,41,55,0.9)' : 'rgba(255,252,245,0.92)'),
            border: isSelected
              ? 'none'
              : isTodayDate
                ? `1.5px solid ${darkMode ? 'rgba(180,150,100,0.5)' : 'rgba(160,128,96,0.4)'}`
                : `0.5px solid ${darkMode ? 'rgba(255,255,255,0.08)' : 'rgba(180,150,100,0.2)'}`,
            ...
            <div key={dk + ':' + idx}>
              ...
              {row.events.map((event) => (
                <button
                  ...
                  className="w-full text-left rounded-xl p-2.5 mb-1 transition-all hover:shadow"
                  style={{ background: darkMode ? 'rgba(255,255,255,0.04)' : 'rgba(255,252,245,0.9)', border: `0.5px solid ${darkMode ? 'rgba(255,255,255,0.08)' : 'rgba(180,150,100,0.2)'}` }}
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5 min-w-0">
                        <span className="w-2 h-2 rounded-full shrink-0" style={{ background: '#c4a882' }} />
                        <span className="text-sm font-medium truncate" style={{ color: darkMode ? '#e5d9c8' : '#3a2e22' }}>{event.title}</span>
                      </div>
                      {event.location && (
                        <div className="text-xs truncate mt-0.5" style={{ color: darkMode ? '#9ca3af' : '#a08060' }}>
                          {event.location}
                        </div>
                      )}
                    </div>
                    <span className="text-xs shrink-0" style={{ color: darkMode ? '#9ca3af' : '#a08060' }}>
        ...
        return (
          <button
            ...
            className={`
              group relative w-full aspect-square rounded-3xl p-2 transition-all duration-300 select-none
              hover:scale-105 active:scale-95
              ${isSelected ? 'shadow-2xl scale-110 z-20' : 'hover:shadow-md'}
            `}
            style={{
              background: isSelected
                ? 'linear-gradient(135deg, #c4a882 0%, #a08060 100%)'
          ...
          >
            ...
            {weatherData && !isSelected && (
              <div className="flex flex-col items-center mb-1">
                ...
                <span className="text-[10px] font-medium" style={{ color: darkMode ? '#9ca3af' : '#a08060' }}>
                  {weatherData.high}°
                </span>
              </div>
            )}
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {!isSelectedToday && (
          <button
            ...
            style={{ color: darkMode ? '#9ca3af' : '#a08060' }}
          >
            Close
          </button>
        )}
      ...
      {events.length > 0 ? (
        <div className="space-y-3 mb-4">
          {events.map((event, idx) => {            const popupMeta = popupEventsByEventId[String(event.id || '')] || null;
            ...
            return (
              ...
              <button
                ...
                className={`group relative z-10 w-full flex items-center gap-4 p-4 rounded-2xl hover:shadow-lg hover:scale-[1.02] transition-all duration-300 text-left`}
                style={{
                  background: isWeEvent
                    ? (darkMode
                    ...
                    : (darkMode ? 'rgba(255,255,255,0.04)' : 'rgba(255,252,245,0.85)'),
                  border: isWeEvent
                    ? `1px solid ${darkMode ? 'rgba(251,113,133,0.5)' : 'rgba(244,63,94,0.3)'}`
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="font-semibold text-base mb-1" style={{ color: darkMode ? '#e5d9c8' : '#3a2e22' }}>
                    {weEventBadge ? `${weEventBadge.icon} ` : ''}{event.title}
                  </div>
                  {event.location && (
                    <div className="flex items-center gap-1 text-sm" style={{ color: darkMode ? '#9ca3af' : '#a08060' }}>
                ...
                {/* Arrow */}
                <div className="text-xl group-hover:translate-x-1 transition-transform" style={{ color: darkMode ? '#b4a080' : '#c4a882' }}>
                  →
                </div>
              </button>
              </div>
