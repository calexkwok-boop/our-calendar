...
function getSuggestionFallbackImagePools() {
  return {
    coffeeUrls: [
...
function isGenericSuggestionFallbackUrl(url) {
  const normalized = String(url || '').trim();
  if (!normalized) return false;
  const pools = getSuggestionFallbackImagePools();
  return Object.values(pools).some((urls) => urls.includes(normalized));
}
...
function SuggestionCardInner({ s, darkMode, shadow }) {
  ...
  const weakSavedImage = isStaticMapSuggestionUrl(savedImageUrl) || isGenericSuggestionFallbackUrl(savedImageUrl);
            ...
            for (const resolvedAnchorLocation of resolvedAnchorLocations) {
              try {
                ...
                for (const keyword of nearbyKeywords) {
                  ...
                  let nearbyCandidates = Array.isArray(nearbyData?.results) ? nearbyData.results : [];
                  if (nearbyCandidates.length === 0) {
                    nearbyCandidates = await fetchLocalNearbyFallback(
                      resolvedAnchorLocation,
                      category,
                      debugSnapshot.localFallbackAttempts
                    );
            ...
            for (const category of LOCAL_BUSINESS_FALLBACK_TYPES) {
              ...
              try {
                ...
                if (candidates.length === 0) {
                  candidates = await fetchLocalNearbyFallback(
                    resolvedAnchorLocation,
                    category,
                    debugSnapshot.localFallbackAttempts
                  );
