...
function buildAutoSortedPins(pins, onAddDream, onDeleteDream, onUpdateDream, startY = 20, chapters = []) {
  ...
  // Chapter pins take precedence over regular categories and are managed by their chapter clusters.
  const chapterPins = pins
    .filter(p => p.type !== 'label' && p.type !== 'sticker' && !p.autoGenerated && getSortChapterId(p))
    .map(p => p.chapterId ? p : { ...p, chapterId: getSortChapterId(p) });
  const contentPins = pins.filter(p => p.type !== 'label' && p.type !== 'sticker' && !p.autoGenerated && p.status !== 'done' && !getSortChapterId(p));
  const donePins    = pins.filter(p => p.type !== 'label' && p.type !== 'sticker' && !p.autoGenerated && p.status === 'done' && !getSortChapterId(p));
