...
const PartyEventCard = ({ event, onUpdateEventData, onEdit, openEditor, ...props }) => {
  ...
  return (
    <div className="group relative overflow-hidden rounded-[32px] border-2 border-orange-300/70 bg-gradient-to-br from-orange-50 via-amber-50/80 to-yellow-50/50 shadow-[0_24px_80px_rgba(251,146,60,0.25)] transition-all duration-300 hover:shadow-[0_28px_100px_rgba(251,146,60,0.35)] dark:border-orange-400/25 dark:from-[#2d1f14] dark:via-[#1c150f] dark:to-[#13110e] dark:shadow-none">
      
      {/* ANIMATED CONFETTI BACKGROUND */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        {/* Large confetti pieces */}
        <div className="absolute left-[10%] top-[15%] h-3 w-3 rotate-12 rounded-sm bg-gradient-to-br from-pink-400 to-rose-400 opacity-40 dark:opacity-20" 
             style={{ animation: 'float 8s ease-in-out infinite' }} />
        <div className="absolute left-[25%] top-[45%] h-2 w-2 rotate-45 rounded-full bg-gradient-to-br from-blue-400 to-cyan-400 opacity-50 dark:opacity-25" 
             style={{ animation: 'float 6s ease-in-out infinite 1s' }} />
        <div className="absolute right-[15%] top-[25%] h-4 w-1 rotate-[30deg] rounded-full bg-gradient-to-b from-yellow-400 to-orange-400 opacity-40 dark:opacity-20" 
             style={{ animation: 'float 7s ease-in-out infinite 0.5s' }} />
        <div className="absolute left-[70%] top-[60%] h-2.5 w-2.5 rotate-[60deg] bg-gradient-to-br from-purple-400 to-pink-400 opacity-40 dark:opacity-20" 
             style={{ animation: 'float 9s ease-in-out infinite 1.5s' }} />
        <div className="absolute right-[30%] top-[70%] h-3 w-3 -rotate-12 rounded-sm bg-gradient-to-br from-green-400 to-emerald-400 opacity-35 dark:opacity-18" 
             style={{ animation: 'float 8.5s ease-in-out infinite 2s' }} />
        <div className="absolute left-[45%] top-[35%] h-2 w-2 rotate-90 bg-gradient-to-br from-orange-400 to-red-400 opacity-45 dark:opacity-22" 
             style={{ animation: 'float 7.5s ease-in-out infinite 0.8s' }} />
        <div className="absolute right-[60%] top-[80%] h-3 w-1 rotate-[15deg] rounded-full bg-gradient-to-b from-pink-400 to-purple-400 opacity-40 dark:opacity-20" 
             style={{ animation: 'float 6.5s ease-in-out infinite 1.2s' }} />
        <div className="absolute right-[20%] top-[50%] h-2 w-2 rotate-[75deg] rounded-sm bg-gradient-to-br from-cyan-400 to-blue-400 opacity-38 dark:opacity-19" 
             style={{ animation: 'float 8s ease-in-out infinite 2.5s' }} />
        
        {/* Small sparkle confetti */}
        <div className="absolute left-[15%] top-[90%] h-1.5 w-1.5 rotate-45 bg-yellow-400 opacity-60 dark:opacity-30" 
             style={{ animation: 'twinkle 3s ease-in-out infinite' }} />
        <div className="absolute right-[25%] top-[20%] h-1 w-1 rotate-12 bg-pink-400 opacity-50 dark:opacity-25" 
             style={{ animation: 'twinkle 2.5s ease-in-out infinite 0.5s' }} />
        <div className="absolute left-[80%] top-[75%] h-1.5 w-1.5 rotate-[30deg] bg-orange-400 opacity-55 dark:opacity-28" 
             style={{ animation: 'twinkle 3.5s ease-in-out infinite 1s' }} />
        <div className="absolute right-[75%] top-[40%] h-1 w-1 rotate-[60deg] bg-purple-400 opacity-45 dark:opacity-23" 
             style={{ animation: 'twinkle 2.8s ease-in-out infinite 1.5s' }} />
      </div>

      {/* DECORATIVE BALLOON STRING (top right corner) */}
      <div className="pointer-events-none absolute right-6 top-0 h-32 w-0.5 bg-gradient-to-b from-orange-300/40 to-transparent dark:from-orange-400/20" 
           style={{ animation: 'sway 4s ease-in-out infinite' }} />
      <div className="pointer-events-none absolute right-5 top-[-8px] h-6 w-6 rounded-full bg-gradient-to-br from-rose-400 to-pink-500 opacity-80 shadow-lg dark:opacity-60" 
           style={{ animation: 'float-balloon 6s ease-in-out infinite' }} />
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <div className="mb-4 flex flex-wrap items-center gap-2.5">
              {/* ANIMATED PARTY ICON */}
              <span className="inline-flex h-12 w-12 items-center justify-center rounded-[18px] border-2 border-orange-300 bg-gradient-to-br from-orange-100 to-amber-100 text-2xl shadow-lg shadow-orange-200/40 transition-transform group-hover:scale-110 group-hover:rotate-12 dark:border-orange-400/30 dark:from-orange-500/15 dark:to-amber-500/15 dark:shadow-orange-500/10"
      ...
      <div className="relative space-y-5 px-6 py-6 sm:px-7">
        ...
        >
          <div className="grid gap-3 sm:grid-cols-2">
            {/* Theme Card with gradient border */}
            <div className="group/card relative overflow-hidden rounded-2xl bg-gradient-to-br from-orange-100 to-orange-50 p-[2px] shadow-sm transition-all hover:shadow-md dark:from-orange-500/20 dark:to-orange-500/10">
              <div className="rounded-[14px] bg-white/90 px-4 py-3.5 dark:bg-gray-900/90">
                ...
                <div className="text-sm font-semibold text-gray-900 dark:text-white">
                  {theme || 'No theme added yet'}
                </div>
              </div>
            </div>
            
            {/* Guest Style Card with gradient border */}
              ...
              {potluckItems.slice(0, 4).map((item, index) => (
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <span className="text-lg leading-none transition-transform group-hover/item:scale-110">🍽️</span>
        ...
        {/* MUSIC - WITH PLAYLIST SERVICE ICONS */}
        {musicPlaylist ? (
          <Section
            title="Music"
            ...
            actions={(
              <>
                {isProbablyUrl(musicPlaylist) && (
                  <ActionPill href={musicPlaylist}>
                    Open {playlistService?.name || 'playlist'}
                  </ActionPill>
                )}
          ...
          >
            <div className="relative overflow-hidden rounded-2xl border-2 border-purple-100 bg-gradient-to-br from-purple-50 to-pink-50 p-4 shadow-sm dark:border-purple-500/10 dark:from-purple-500/5 dark:to-pink-500/5">
              ...
              <div className="pointer-events-none absolute right-4 top-2 text-4xl opacity-10 dark:opacity-5">🎵</div>
                  ...
                  {isProbablyUrl(musicPlaylist) ? (
                    <div className="flex flex-col gap-1.5">
                      ...
                      <span className="text-xs text-gray-600 dark:text-gray-400">
                        Tap the button to open the playlist.
                      </span>
                    </div>
                  ) : (
      ...
      {/* CSS Animations - Add to global stylesheet */}
      <style jsx>{`
        @keyframes float {
